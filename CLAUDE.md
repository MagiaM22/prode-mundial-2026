...
- **Notas del partido (1-10): FotMob** (`www.fotmob.com/api/data`, CORS abierto, sin key; ojo: la ruta es `/api/data/...`, `/api/...` a secas da 404). `mergeFotmobRatings` mezcla la nota en las stats por slug del nombre (con fallback de tokens ordenados porque FotMob invierte nombres asiáticos: "In-Beom Hwang") y marca `is_mvp` al mejor puntuado → ahí aplica el +2 del scoring. FotMob agrupa por fecha UTC: se busca la fecha del fixture y el día siguiente. Best effort: si FotMob no responde, el sync sigue sin notas. Requiere columna `rating numeric` en `gdt_match_stats`.
